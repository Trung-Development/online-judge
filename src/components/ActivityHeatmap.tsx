...
export default function ActivityHeatmap({ submissions }: ActivityHeatmapProps) {
  ...
  useEffect(() => {
    ...
    submissions.forEach((submission) => {
      const date = new Date(Number(submission.timestamp))
        .toISOString()
        .split("T")[0];
      if (!data[date]) {
        data[date] = 0;
  ...
  useEffect(() => {
    ...
    const drawContribution = (year: number) => {
      ...
      document
        .querySelectorAll("#submission-activity-table td")
      ...
      if (prevYearAction) {
        prevYearAction.style.display =
          year > new Date().getFullYear() - 5 ? "" : "none";
      }

      if (nextYearAction) {
        nextYearAction.style.display =
          year < new Date().getFullYear() ? "" : "none";
      }
      ...
      for (
        ...
        current_weekday < days[0].weekday;
        current_weekday++
      ) {
      ...
      const maxActivity = Math.max(
        1,
        Math.max(...days.map((obj) => obj.activity)),
      );
        ...
        if (row) {
          ...
          cell.addEventListener("mouseenter", function () {
            ...
            const tooltipDirection =
              dayNum < 183 ? "tooltipped-w" : "tooltipped-e";
  ...
  return (
    <div className="activity-heatmap" ref={containerRef}>
      <h4 id="submission-activity-header">
        {visibleSubmissionsCount} submissions in{" "}
      ...
      <div id="submission-activity" style={{ display: "block" }}>
        <div id="submission-activity-actions">
          <a
            ...
            style={{
              display:
                currentYear <= new Date().getFullYear() - 5 ? "none" : "",
            }}
          >
            &laquo;
          </a>
          ...
          <a
            ...
            style={{
              display: currentYear >= new Date().getFullYear() ? "none" : "",
            }}
          >
            &raquo;
          </a>
        ...
        <div id="submission-activity-display">
          <table
            id="submission-activity-table"
            className="submission-activity-table"
            cellSpacing="1"
          >
            <tbody>
              {/* Day of week labels in rows */}
              <tr id="submission-0">
                <th className="submission-date-col info-text">Sun</th>
                {/* Cells will be inserted here via JS */}
              </tr>
              <tr id="submission-1">
                <th className="submission-date-col info-text">Mon</th>
                {/* Cells will be inserted here via JS */}
              </tr>
              <tr id="submission-2">
                <th className="submission-date-col info-text">Tue</th>
                {/* Cells will be inserted here via JS */}
              </tr>
              <tr id="submission-3">
                <th className="submission-date-col info-text">Wed</th>
                {/* Cells will be inserted here via JS */}
              </tr>
              <tr id="submission-4">
                <th className="submission-date-col info-text">Thu</th>
                {/* Cells will be inserted here via JS */}
              </tr>
              <tr id="submission-5">
                <th className="submission-date-col info-text">Fri</th>
                {/* Cells will be inserted here via JS */}
              </tr>
              <tr id="submission-6">
                <th className="submission-date-col info-text">Sat</th>
                {/* Cells will be inserted here via JS */}
              </tr>
            </tbody>
          </table>
            ...
            <table className="info-table" cellSpacing="1">
              <tbody>
                <tr>
                  ...
                  <td className="activity-0">
                    <div></div>
                  </td>
                  <td className="activity-1">
                    <div></div>
                  </td>
                  <td className="activity-2">
                    <div></div>
                  </td>
                  <td className="activity-3">
                    <div></div>
                  </td>
                  <td className="activity-4">
                    <div></div>
                  </td>
