...
export default function UnlockLockedProblem({ problem, sessionToken, user, isAuthenticated }: { problem: IProblemPageData, sessionToken?: string, user?: User, isAuthenticated?: boolean }) {
    const canUnlock = isAuthenticated && (problem.author.includes(user?.username || "") || problem.curator.includes(user?.username || "") || hasPermission(user?.perms, UserPermissions.LOCK_PROBLEM));
            ...
            if (!res.ok) {
                res.json().then(v => {
                    ...
                    if (v?.error) alert(`Failed to unlock problem: ${v.error}`);
                        ...
                        alert(`Failed to load the problem: ${res.status}. More details are available in the console.`);
                    }
                }).catch((x) => {
                    alert(`Failed to load the problem: ${res.status}. More details are available in the console`);
                    console.log(x);
    ...
    return (
        <main className="max-w-4xl mx-auto py-8 px-4">
            <Alert variant="warning" className="mb-6">
                ...
                <AlertDescription>
                    {canUnlock ? "This problem is currently locked. Please unlock it to allow modifications." : "Modifications to this problem are restricted. Please contact an administrator for further assistance."}
                </AlertDescription>
                {canUnlock && <Button
