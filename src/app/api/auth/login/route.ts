...
export async function POST(request: NextRequest) {
  try {
    const { email, password, captchaToken }: LoginCredentials =
      await request.json();
    ...
    if (!sessionRes.ok) {
      ...
      if (errorBody.message === "ACCOUNT_AWAITING_CONFIRMATION") {
        return NextResponse.json(
          {
            error:
              "Your account hasn't been confirmed yet. Please check your email or contact an administrator.",
          },
          { status: 401 },
        );
      }
      if (
        errorBody.message === "ACCOUNT_BANNED" ||
        errorBody.message === "ACCOUNT_DISABLED"
      ) {
        return NextResponse.json(
          {
            error:
              "Your account isn't active. Please contact an administrator for more information.",
          },
          { status: 401 },
        );
      }
